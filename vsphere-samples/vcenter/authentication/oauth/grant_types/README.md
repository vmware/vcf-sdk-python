...
## Login Steps
1.  From a given VC IP/hostname, find the Identity Provider (sample available at [list_external_identity_providers.py](../list_external_identity_providers.py))
...
4.  Convert access token to saml token (sample avaialble at [exchange_access_id_token_for_saml.py](../exchange_access_id_token_for_saml.py))
...
First start the webserver code at [webserver.py](./webserver.py). Note, this server is not recommended in a production setting, this is only to demonstarte the sample workflow
