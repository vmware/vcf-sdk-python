...
class CustomizeVMWithCloudinitData(object):
    ...
    Enable cloud-init customization in the cloud.cfg file on the Linux VM.
    Details refer to [Guest Customization Using cloud-init](https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere-sdks-tools/7-0/web-services-sdk-programming-guide/virtual-machine-guest-operations/guest-customization-using-cloud-init.html)
    """
    def __init__(self):
        self.metadata = None
        self.userdata = None
    ...
    def _getVmRef(self, vmName=None):
        ...
        return pchelper.get_obj(content, [vim.VirtualMachine], vmName)
    ...
    def createCloudinitDataSpec(self):
        ...
        metadataYamlFilePath = os.path.join(os.path.dirname(
                                            ...
                                            'resources/sample_metadata.yaml')
        userdataFilePath = os.path.join(os.path.dirname(
                                        os.path.realpath(__file__)),
                                        'resources/sample_userdata')
