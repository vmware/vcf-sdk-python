...
def verifyPrecheckResult(result):
   """
...
def applyVsanConfig(si, vsccs, cluster, vsanConfig, dsConfig, dsName):
...
def main():
   ...
   if aboutInfo.apiType == 'VirtualCenter':
      ...
      Mount: The provided remote vSAN datastore(s) will be mounted to the client
             cluster. If the client cluster is already mounted, then it will be
             skipped.
      ...
      localDatastore = vsanapiutils.GetLocalVsanDatastore(serverCluster)
      ...
      if len(localDatastore) > 0:
         ...
         if verifyPrecheckResult(result):
            ...
            dsConfig = vim.vsan.AdvancedDatastoreConfig(
                          remoteDatastores=[localDatastore[0]])
            applyVsanConfig(si, vsccs, clientCluster, vsanConfig,
                            dsConfig, localDatastore[0].name)
