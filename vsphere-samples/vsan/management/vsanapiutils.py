...
from pyVmomi import vim, vmodl, SoapStubAdapter, VmomiSupport, pbm
import pyVmomi
...
def IsEsaAutoRaidEnabledInCluster(clusterConfigs):
   """
   Check if the 'Apply Auto-RAID to all objects' configuration is enabled in the cluster.
   :param clusterConfigs: The cluster configuration.
   :return: True if enabled, False otherwise.
   """
   assert 'VSAN_ESX_MODE' not in os.environ
   if not clusterConfigs:
      return False
   vsanEsaEnabled = clusterConfigs.vsanEsaEnabled

   if not vsanEsaEnabled:
      return False
   else:
      vsanEsaConfigInfo = clusterConfigs.vsanEsaConfigInfo
      if vsanEsaConfigInfo:
         autoRAIDConfig = None
         if hasattr(vsanEsaConfigInfo, 'autoRAIDConfig') and \
            vsanEsaConfigInfo.autoRAIDConfig:
            autoRAIDConfig = vsanEsaConfigInfo.autoRAIDConfig
         if hasattr(autoRAIDConfig, 'assumeAutoManagedRAID') and \
            autoRAIDConfig.assumeAutoManagedRAID:
            return autoRAIDConfig.assumeAutoManagedRAID
      return False

def getClusterUuid(cluster):
   vsanConfigInfo = cluster.configurationEx.vsanConfigInfo
   if vsanConfigInfo.enabled == False:
      print('Cluster is not vSAN enabled')
      return -1
   return vsanConfigInfo.defaultConfig.uuid
...
def GetLocalVsanDatastore(cluster):
   ...
   @param cluster the given cluster reference
...
def ConnectToSpbm(stub, context):
   """
   Connect to the Storage Policy Based Management (SPBM) service.
   :param stub: The vCenter or ESXi stub.
   :param context: The SSL context for secure connections.
   :return: The pbm service instance.
   """
   sessionCookie = stub.cookie.split('"')[1]
   VmomiSupport.GetRequestContext()["vcSessionCookie"] = sessionCookie
   pbmStub = _GetVsanStub(stub, endpoint="/pbm/sdk", context=context,
                          version="pbm.version.version2")
   pbmStub.cookie = stub.cookie
   pbmSi = pbm.ServiceInstance("ServiceInstance", pbmStub)
   return pbmSi

def GetVsanStorageProfile(pbmSi, vsanDatastore):
   """
   Get the vSAN storage profile for the given cluster.
   :param pbmSi: The pbm service instance.
   :param vsanDatastore: The vSAN datastore object.
   :return: The vSAN storage profile object.
   """
   ...
   vsanDefaultProfile = profileManager.PbmQueryDefaultRequirementProfile(
      hub = pbm.placement.PlacementHub(hubType="Datastore", hubId=vsanDatastore._moId)
      )
   vsanDefaultProfileId = vsanDefaultProfile.uniqueId
   print("vSAN datastore default profile UUID is: %s" % vsanDefaultProfileId)
   # Retrieve all profiles and find the vSAN storage profile with the same UUID.
   ...
   for profile in profiles:
      ...
      if (isinstance(profile, pbm.profile.CapabilityBasedProfile) and
            profileId == vsanDefaultProfileId):
         print("Found vSAN storage profile: %s" % profileId)
         return profile
   return None

def IsAutoManagedRAIDEnabledInProfile(profile):
   """
   Check if the vSAN storage profile has auto-managed RAID enabled.
   :param profile: The vSAN storage profile object.
   :return: True if auto-managed RAID is enabled, False otherwise.
   """
   constraints = getattr(profile, 'constraints', None)
   if not constraints:
      print("No constraints found in the profile")
      return False

   subProfiles = getattr(constraints, 'subProfiles', [])
   subProfiles = subProfiles or []
   for subProfile in subProfiles:
      capabilities = subProfile.capability or []
      for cap in capabilities:
         if not cap.id:
            continue
         if cap.id.namespace.lower().strip() != 'vsan':
            return False
         if cap.id.id.lower().strip() != 'automanagedraid':
            continue
         if not cap.constraint:
            return False
         cons = cap.constraint[0]
         propertyInstance = cons.propertyInstance
         if not propertyInstance:
            return False
         prop = propertyInstance[0]
         return prop.value
   return False