...
def main():
    ...
    spec.host = firstHost
    eligibleVsanDisks = queryEligibleVsanDisks(firstHost)
    print("Eligible vSAN disks:", [d.canonicalName for d in eligibleVsanDisks],
          "on host", firstHost)

    if len(eligibleVsanDisks) > 0:
    ...
    #        storage pool.
    secondHost = hosts[1]
    eligibleVsanDisks = queryEligibleVsanDisks(secondHost)
    print("Eligible vSAN disks:", [d.canonicalName for d in eligibleVsanDisks],
          "on host", secondHost)

    spec = vim.vsan.host.AddStoragePoolDiskSpec()
    spec.host = secondHost
    ...
    if len(storagePoolDisks) > 0:
        storagePoolDisks = queryStoragePoolDisks(vdms, secondHost)
        spec = vim.vsan.host.DeleteStoragePoolDiskSpec()
        mspec = vim.host.MaintenanceSpec(
            vsanMode=vim.vsan.host.DecommissionMode(objectAction=DECOMISSION_MODE))
        spec.diskUuids = [storagePoolDisks[0].disk.vsanDiskInfo.vsanUuid]
