...
class VCFIntegrationConfig:
    def __init__(self, name="Adapter-Instance", description="Adapter instance for illustrative purposes",
                 collectorId="1", credentialConfig=None, identifiers=None, force=False, forceManagementOwnership=False):
        """
        Configuration for VCF integration examples
        ...
        if missing:
            raise ValueError(f"Invalid configuration for VCFIntegrationConfig: missing {', '.join(missing)}")
