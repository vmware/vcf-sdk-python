...
from utils.sddc_spec_util import WorkflowType, VERSION, \
    hostname_to_fqdn, \
    AUTO_GENERATED_PASSWORD, create_vcf_operations_spec_HA, \
    ...
    create_vcf_automation_spec_HA, create_ip_address_pool_spec, \
    save_sddc_spec_to_file, create_vidb_spec, create_vsp_cluster_spec, create_vsp_ipv4_pool, \
...
parser.add_argument(
    '--vcf_ops_fqdn1',
    required=True,
    help='First hostname or FQDN of the VCF Operations that will be deployed. Example: vcfops1')

parser.add_argument(
    '--vcf_ops_fqdn2',
    required=True,
    help='Second hostname or FQDN of the VCF Operations that will be deployed. Example: vcfops2')

parser.add_argument(
    '--vcf_ops_fqdn3',
    required=True,
    help='Third hostname or FQDN of the VCF Operations that will be deployed. Example: vcfops3')

parser.add_argument(
    '--vcf_automation_fqdn',
    help='Hostname or FQDN of the VCF Automation that will be deployed. If left None along with'
         '--vcf_automation_pool_ip_node_1 to --vcf_automation_pool_ip_node_4 - VCF '
         'Automation deployment is skipped.')

parser.add_argument(
    '--vcf_automation_pool_ip_node_1',
    help='First IP node of the VCF Automation. If left None along with'
         '--vcf_automation_fqdn and --vcf_automation_pool_ip_range_X - VCF '
         'Automation deployment is skipped.')

parser.add_argument(
    '--vcf_automation_pool_ip_node_2',
    help='Second IP node of the VCF Automation. If left None along with'
         '--vcf_automation_fqdn and --vcf_automation_pool_ip_range_X - VCF '
         'Automation deployment is skipped.')

parser.add_argument(
    '--vcf_automation_pool_ip_node_3',
    help='Third IP node of the VCF Automation. If left None along with'
         '--vcf_automation_fqdn and --vcf_automation_pool_ip_range_X - VCF '
         'Automation deployment is skipped.')

parser.add_argument(
    '--vcf_automation_pool_ip_node_4',
    help='Fourth IP node of the VCF Automation. If left None along with'
         '--vcf_automation_fqdn and --vcf_automation_pool_ip_range_X - VCF '
         'Automation deployment is skipped.')
...
def create_sddc_spec_for_new_vcf_fleet(vcf_client, args):
    ...
    # Operations stack
    spec.vcf_operations_spec = create_vcf_operations_spec_HA(
        hostname_to_fqdn(args.vcf_ops_fqdn1, args.dns_domain),
        hostname_to_fqdn(args.vcf_ops_fqdn2, args.dns_domain),
        hostname_to_fqdn(args.vcf_ops_fqdn3, args.dns_domain),
    ...
    # VCF Automation
    spec.vcf_automation_spec = create_vcf_automation_spec_HA(
        hostname_to_fqdn(args.vcf_automation_fqdn, args.dns_domain),
        args.vcf_automation_platform_fqdn,
        args.vcf_automation_pool_ip_node_1,
        args.vcf_automation_pool_ip_node_2,
        args.vcf_automation_pool_ip_node_3,
        args.vcf_automation_pool_ip_node_4)
