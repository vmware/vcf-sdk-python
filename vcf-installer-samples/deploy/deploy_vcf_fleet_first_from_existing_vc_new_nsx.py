...
from utils.sddc_spec_util import WorkflowType, VERSION, \
    ...
    create_sddc_nsxt_spec, create_sddc_manager_spec, \
...
"""
Description:
Demonstrates how to deploy new VCF Fleet with existing VC and new NSX.
In addition to that deploy VSP components.
Prerequisites for successful deployment:
    1. Existing vcenter component need to be configured and reachable by the VCF Installer appliance
...
parser.add_argument(
    "--vsp_ipv4_addresses",
    help="VCF Services Platform IPv4 addresses. All IPv4 fields are optional,"
         "however IPv4Pool is required for the VCF Services Platform cluster spec."
         "Either provide vsp_ipv4_addresses or provide"
         "vsp_ipv4_cidr, or vsp_ipv4_start_ip_address and vsp_ipv4_end_ip_address.")
...
def create_sddc_spec_for_vcf_fleet_existing_vc_new_nsx(vcf_client, args):
    ...
    # VCF Automation
    if args.vcf_automation_fqdn and args.vcf_automation_ipv4_addresses and args.vcf_automation_platform_fqdn:
...
sddc_spec = create_sddc_spec_for_vcf_fleet_existing_vc_new_nsx(client, args)
